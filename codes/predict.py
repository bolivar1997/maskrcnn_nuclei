...
if __name__ == '__main__':
    ...
    with tf.device(DEVICE):
        model = get_model(config = config, model_path='../mask_rcnn.h5')
    ...
    count = 0
    for img_path in sorted(glob.glob('../images_256/*.jpg')):
        ...
        first = masks[:, :, 0]
        result = np.array(first).astype(np.float32)

        for i in range(1, masks.shape[2]):
            first = masks[:, :, i]
            first = np.array(first).astype(np.float32)

            result[first > 0] = 1



        misc.imsave('../result/%d.png' % id, result)
